...
gdzie: d(x, y) - odległość pomiędzy wektorem x i y; x, y - wektory o tym samym rozmiarze; n - rozmiar wektorów x i y;  x\textsubscript{n}, y\textsubscript{n} - składowe wektora. 
\\\\
By móc obliczyć odległość pomiędzy wektorami cech zadanych tekstów, należy wcześniej skorzystać z miary podobieństwa tekstu by zamienić cechy o wartościach tekstowych na liczby w wektorach. W programie została zastosowana metoda bigramów !!!!!!!!!!!!!!Źródło!!!!!!!!!!!!!!!. Korzystając z tej metody obliczamy współczynnik podobieństwa dla dwu tekstów tej samej cechy tekstowej zgodnie ze wzorem:
\begin{equation} s = \frac {2n_t}{n_1 + n_2} \end{equation}

Gdzie s - wartość liczbowa będąca podobieństwem dwu tekstów zawierająca się w przedziale [0, 1], n\textsubscript{t} - liczba bigramów powatrzających się w obu tekstach,  n\textsubscript{1} - liczba bigramów utworzonych z pierwszego tekstu,  n\textsubscript{2} - liczba bigramów utworzonych z drugiego tekstu \\
\newline
Przykład:\\
...
\begin{equation} s = \frac {2 * 1}{4 + 4} = \frac{1}{4} \end{equation}

Obliczone w ten sposób podobieństwo dwu cech tekstowych wykorzystywane jest do obliczenia odległości pomiędzy nimi:
