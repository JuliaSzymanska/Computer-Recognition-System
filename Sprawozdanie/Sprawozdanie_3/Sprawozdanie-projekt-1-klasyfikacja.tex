...
\begin{equation} PPV_{calk} = \sum_{n=1}^{m} (PPV_n *\frac{TP + FN}{TP + TN + FP + FN}) \end{equation}
...
\begin{equation} TPR_{calk} = \sum_{n=1}^{m} (TPR_n *\frac{TP + FN}{TP + TN + FP + FN}) \end{equation}
Gdzie TPR\textsubscript{calk} - czułość obliczona dla wszystkich klas klasyfikowanych dokumentów,  m - liczba rozpatrywanych klas, TPR\textsubscript{n} - czułość dla n-tej klasy\\
